...
### The wake's mirror image: cancelling a parent mid-wait

Every path above is about making sure a blocked step *does* get woken. There is a matching hazard pointing the other way, and it was a real bug caught in review rather than a hypothetical: a blocked step must also stop existing when its own run is cancelled.

Run cancellation (`cancelPendingSteps`, and `finalizeCancelledRun`'s equivalent update) originally swept steps `where status in ('pending', 'ready')`. That list is a decision about which steps nobody is currently responsible for — `running` steps are deliberately excluded, because their worker still owns their outcome and its fenced commit should win. A `blocked` step reads, at a glance, like it belongs in neither group: it has no lease, no worker is executing it, nothing is in flight. It looks *finished*.

It isn't. It's suspended, and something is going to come back for it. So cancelling a parent mid-await left the step sitting in `blocked`, in a run now marked `cancelled` — and then the child ran to completion, its terminal transition fired the wake exactly as designed, and the step flipped to `ready`: a claimable step inside a run that was already over. The worker's "run was cancelled before this step started" guard meant the step function never actually executed, so the blast radius was wasted claims rather than incorrect work — but the row was lying about the state of the system, which is the thing this engine's whole design refuses to do.

The fix is to treat `blocked` like `pending` for cancellation purposes — cancel it with the rest, and clear `awaited_child_run_id` on the way out so the wake's `where status = 'blocked'` guard can no longer match it. The general lesson is worth stating plainly, because Phase 5 adds another suspended status (`waitForEvent`) that will walk straight into it: **every status that means "suspended, someone will come back for it" has to be enumerated in both directions** — in the wake path that resumes it, and in the cancellation path that tears it down. Adding a suspend status is never a one-sided change.
...
**Cancellation does not cascade to children.** Cancelling a parent run cancels the parent's own steps, including the blocked one awaiting a child (§4) — but the child run itself keeps going to completion, unaware that nobody is waiting for its result any more. That's an orphan, and it's a deliberate omission rather than a missed case: cascading cancellation down a child tree needs a decision about how far it propagates (grandchildren too?), whether a child gets to run compensation before it dies, and what happens when the child is mid-step with a live lease — which is compensation-and-teardown territory, i.e. Phase 7. Until then, a cancelled parent leaves its children running, and cancelling those is the caller's job (`cancelRun` on the child run id, which `getChildRuns` will give you).
